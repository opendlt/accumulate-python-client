...
def compute_hash(obj: Any) -> bytes:
    """
    Compute a SHA-256 hash for an object implementing a `marshal_binary`-like method.
    """
...
def LiteAuthorityForKey(pub_key: bytes, signature_type: str) -> str:
    """
    Generate a lite authority URL for a given public key and signature type.

    :param pub_key: The public key as bytes.
    :param signature_type: The signature type (e.g., "ED25519", "BTC").
    :return: A string representing the authority URL for the lite account.
    """
    key_hash = public_key_hash(pub_key, signature_type)
    return LiteAuthorityForHash(key_hash)


def LiteAuthorityForHash(key_hash: bytes) -> str:
    """
    Generate a lite authority URL for a given key hash.

    :param key_hash: The key hash as bytes.
    :return: A string representing the authority URL for the lite account.
    """
    key_str = key_hash[:20].hex()
    checksum = hashlib.sha256(key_str.encode()).digest()[-4:].hex()
    return f"{key_str}{checksum}"
